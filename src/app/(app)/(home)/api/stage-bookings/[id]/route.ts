...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const booking = await db.collection("stageBookings").findOne({ _id: new ObjectId(bookingId) });
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existingBooking = await db.collection("stageBookings").findOne({ _id: new ObjectId(bookingId) });
    ...
    if (stageDetails) {
      updateData.stageDetails = {
        ...
        expectedGuests:
          stageDetails.expectedGuests !== undefined
            ? stageDetails.expectedGuests
            : existingBooking.stageDetails.expectedGuests,
        specialRequirements:
          stageDetails.specialRequirements !== undefined
            ? stageDetails.specialRequirements
            : existingBooking.stageDetails.specialRequirements || "",
      };
    }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await db.collection("stageBookings").deleteOne({ _id: new ObjectId(bookingId) });
