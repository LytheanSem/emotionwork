...
import { Upload, FileText, Image, Trash2, ShoppingCart, Package } from "lucide-react";
...
interface StageBookingFormData {
  ...
  designFiles: File[];
  equipmentItems?: any[]; // Equipment items from cart
}
...
export function StageBookingForm({ onSubmit }: StageBookingFormProps) {
  ...
  const { cartItems, getCartItemCount, getTotalPrice } = useCart();
  ...
  const [formData, setFormData] = useState<StageBookingFormData>({
    ...
    stageDetails: {
      location: "",
      eventType: "",
      eventDates: [], // Changed from eventDate to eventDates array
      eventTime: "",
      duration: 4,
      expectedGuests: 50,
      specialRequirements: "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formDataToSubmit = {
      ...
      equipmentItems: cartItems, // Include equipment items from cart
    };
          ...
          {cartItems.length > 0 ? (
            ...
            <div className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Need equipment for your event? Browse our professional lighting, sound, and staging equipment.
                </AlertDescription>
              </Alert>
